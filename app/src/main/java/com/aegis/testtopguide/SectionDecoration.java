...
public class SectionDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        String groupId = decorationCallback.getGroupId(pos);
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            String groupId = decorationCallback.getGroupId(position);
            if (groupId.equals("-1")) return;
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position + 1 < itemCount) {
                //在 position后面+1 可以进行每一个条目都进行一次滑动
                String nextGroupId = decorationCallback.getGroupId(position);
                //组内最后一个view进入了header
    ...
    private boolean isFirstInGroup(int pos) {
        if (pos == 0) {
            ...
            String prevGroupId = decorationCallback.getGroupId(pos - 1);
            String groupId = decorationCallback.getGroupId(pos);
